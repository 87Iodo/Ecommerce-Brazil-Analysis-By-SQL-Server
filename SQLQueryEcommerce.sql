...
SELECT * FROM
(SELECT pt.product_category_name_english, oi.product_id, COUNT(order_id) as total_sales_per_product, 
	ROW_NUMBER() OVER(PARTITION BY product_category_name_english ORDER BY pt.product_category_name_english, COUNT(order_id) DESC) AS Popularity
FROM order_items oi
INNER JOIN products p on oi.product_id = p.product_id
INNER JOIN product_category_name_translation pt on p.product_category_name = pt.product_category_name
GROUP BY pt.product_category_name_english, oi.product_id) AS POP
WHERE Popularity <=3
;
...
BEGIN
	...
	END
	--Query 11
	ELSE IF @QueryType = 'Top3PopularProduct'
	BEGIN
	SELECT * FROM
	(SELECT pt.product_category_name_english, oi.product_id, COUNT(order_id) as total_sales_per_product, 
		ROW_NUMBER() OVER(PARTITION BY product_category_name_english ORDER BY pt.product_category_name_english, COUNT(order_id) DESC) AS Popularity
